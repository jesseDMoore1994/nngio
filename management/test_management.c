...
void test_service_discovery() {
  ...
  LibnngioProtobuf__ServiceDiscoveryRequest *recv_request = NULL;
  proto_rv = libnngio_protobuf_recv_service_discovery_request(
    mgmt->proto_ctx,
    &recv_request);

  libnngio_log("INF", "TEST_SERVICE_DISCOVERY", __FILE__, __LINE__, -1,
               "Service discovery request received by server, result: %d", proto_rv);
  ...
  libnngio_log("INF", "TEST_SERVICE_DISCOVERY", __FILE__, __LINE__, -1,
               "Service discovery response created by server");

  libnngio_protobuf_send_service_discovery_response(
    mgmt->proto_ctx, response);
  ...
  LibnngioProtobuf__ServiceDiscoveryResponse *recv_response = NULL;
  proto_rv = libnngio_protobuf_recv_service_discovery_response(
      client_proto_ctx, &recv_response);
  ...
  // Free received request and response
  if (recv_request) {
    libnngio_protobuf__service_discovery_request__free_unpacked(recv_request, NULL);
  }
  if (recv_response) {
    nngio_free_service_discovery_response(recv_response);
  }
...
void test_service_discovery_async() {
  ...
  // Prepare async state
  async_test_state recv_state = {0}, send_state = {0};

  libnngio_protobuf_recv_cb_info recv_cb_info = {
    .user_cb = async_recv_cb,
    .user_data = &recv_state
  };
  ...
  LibnngioProtobuf__ServiceDiscoveryRequest *recv_request = NULL;
  proto_rv = libnngio_protobuf_recv_service_discovery_request_async(
    mgmt->proto_ctx,
    &recv_request,
    recv_cb_info);
  assert(proto_rv == LIBNNGIO_PROTOBUF_ERR_NONE);
  ...
  libnngio_log("INF", "TEST_SERVICE_DISCOVERY_ASYNC", __FILE__, __LINE__, -1,
               "Async service discovery request negotiated, starting response...");
  ...
  mock_response = nngio_copy_service_discovery_response(response);
  LibnngioProtobuf__LibnngioMessage *mock_response_msg =
      nngio_create_nngio_message_with_service_discovery_response(
          "uuid-discovery", mock_response);
  size_t packed_size =
  ...
  proto_rv = libnngio_protobuf_recv_service_discovery_response_async(
    client_proto_ctx, &recv_response, recv_cb_info);
  assert(proto_rv == LIBNNGIO_PROTOBUF_ERR_NONE);

  proto_rv = libnngio_protobuf_send_service_discovery_response_async(
    mgmt->proto_ctx, response, send_cb_info);
  assert(proto_rv == LIBNNGIO_PROTOBUF_ERR_NONE);
...
void test_rpc_service_invoke_sync() {
  ...
  // Receive RPC request on server side
  LibnngioProtobuf__RpcRequest *recv_request = NULL;
  proto_rv = libnngio_protobuf_recv_rpc_request(
    mgmt->proto_ctx,
    &recv_request);
  assert(proto_rv == LIBNNGIO_PROTOBUF_ERR_NONE);
  ...
  // Create RPC response
  LibnngioProtobuf__RpcResponse *response = NULL;
  proto_rv = libnngio_server_create_rpc_response(
    mgmt, rpc_request,  &response);
  assert(proto_rv == LIBNNGIO_PROTOBUF_ERR_NONE);

  //log response fields for debugging
  libnngio_log("INF", "TEST_RPC_SERVICE_INVOKE_SYNC", __FILE__, __LINE__, -1,
               "RPC Response created with status: %d", response->status);
  libnngio_log("INF", "TEST_RPC_SERVICE_INVOKE_SYNC", __FILE__, __LINE__, -1,
               "RPC Response payload length: %zu", response->payload.len);
  ...
  // Send RPC response
  proto_rv = libnngio_protobuf_send_rpc_response(
    mgmt->proto_ctx, response);
  assert(proto_rv == LIBNNGIO_PROTOBUF_ERR_NONE);
  
  // Receive RPC response
  LibnngioProtobuf__RpcResponse *recv_response = NULL;
  proto_rv = libnngio_protobuf_recv_rpc_response(client_proto_ctx, &recv_response);
  
  if (proto_rv == LIBNNGIO_PROTOBUF_ERR_NONE && recv_response != NULL) {
    libnngio_log("INF", "TEST_RPC_SERVICE_INVOKE_SYNC", __FILE__, __LINE__, -1,
                 "RPC call succeeded! Status: %d", recv_response->status);
    
    // The response payload should contain LibnngioManagement__ListTransportsResponse
    if (recv_response->status == LIBNNGIO_PROTOBUF__RPC_RESPONSE__STATUS__Success) {
      LibnngioManagement__ListTransportsResponse *list_resp =
          libnngio_management__list_transports_response__unpack(
              NULL, recv_response->payload.len, recv_response->payload.data);
  ...
  nngio_free_rpc_response(recv_response);
  nngio_free_rpc_request(rpc_request);
  nngio_free_rpc_request(recv_request);
...
void test_rpc_service_invoke_async() {
  ...
  // Prepare async state
  async_test_state send_state = {0}, recv_state = {0};
  ...
  proto_rv = libnngio_protobuf_recv_rpc_request_async(
    mgmt->proto_ctx,
    &recv_request,
    recv_cb_info);
  assert(proto_rv == LIBNNGIO_PROTOBUF_ERR_NONE);
  ...
  libnngio_server_create_rpc_response(
    mgmt, rpc_request,  &rpc_response);

#ifdef NNGIO_MOCK_TRANSPORT
  // Mock service discovery response
  LibnngioProtobuf__RpcResponse *mock_response = nngio_copy_rpc_response(rpc_response);
  LibnngioProtobuf__LibnngioMessage *mock_response_msg =
      nngio_create_nngio_message_with_rpc_response(
          "uuid-rpc", mock_response);
  ...
  //Send RPC response
  proto_rv = libnngio_protobuf_send_rpc_response_async(
    mgmt->proto_ctx, rpc_response, send_cb_info);
  assert(proto_rv == LIBNNGIO_PROTOBUF_ERR_NONE);
  ...
  if (rpc_response->status == LIBNNGIO_PROTOBUF__RPC_RESPONSE__STATUS__Success) {
    ...
    if (list_resp != NULL) {
      ...
      // We expect all 4 services
      assert(list_resp->n_services == 4);
